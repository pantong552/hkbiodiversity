...
export default function FungiSpeciesManager() {
  ...
  // Fetch Metadata (Unique Groups from fungi_species)
  useEffect(() => {
    const fetchMetadata = async () => {
      ...
      while (true) {
        ...
        if (error) {
          console.error('Error fetching fungi taxa groups:', error);
          break;
        }
  ...
  // Fetch Species Data (All at once from fungi_species for better sorting/filtering performance)
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error fetching fungi species data:', err);
    } finally {
      setLoading(false);
    }
  }, [supabase]);
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      // Natural Sort for taxa_id (Handle "fungi_" or "fauna_")
      if (sortConfig.key === 'taxa_id') {
        const aNum = parseInt(String(aVal).replace(/^(fungi_|fauna_)/, ''), 10) || 0;
        const bNum = parseInt(String(bVal).replace(/^(fungi_|fauna_)/, ''), 10) || 0;
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      console.error('Error saving fungi species edit:', err);
  ...
  const filterOptions = useMemo(() => {
    ...
    Object.keys(columnWidths).forEach(key => {
      ...
      options[key] = Object.entries(counts).map(([name, count]) => {
        ...
        // Use translation for fungi taxonomy ranks if bilingual is on
        if (isBilingual && ['class_eng', 'order_eng', 'family_eng', 'genus_eng', 'species_eng', 'informal_group_eng'].includes(key) && name) {
          ...
          const translated = getTaxonomyChi(rank, 'fungi', name);
                  ...
                  >
                    ...
                    <div className="max-h-64 overflow-y-auto space-y-0.5 custom-scrollbar">
                      {uniqueGroups.map(group => (
                        ...
                        >
                          {language === 'zh' 
                            ? getTaxonomyChi('taxa_group', 'fungi', group) 
                            : group
                          }
                    ...
                    {paginatedData.map((item) => {
                      ...
                      return (
                          ...
                          {!isDetailEditMode && (
                            <td className="px-4 py-2 text-[12px] font-medium text-slate-700">
                              {isEditing ? (
                                ...
                                >
                                  {getTaxonomyChi('informal_group_eng', 'fungi', item.informal_group_eng)}
                                </motion.span>
                              )}
                        ...
                        {!isDetailEditMode && ['class_eng', 'order_eng', 'family_eng', 'genus_eng', 'species_eng'].map((key) => {
                          ...
                          const chineseValue = (isBilingual && key !== 'species_eng') ? getTaxonomyChi(rank, 'fungi', englishValue) : null;
                          ...
                          <td className="px-4 py-3 text-right">
                            <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                              <button 
                                onClick={saveEdit}
                                disabled={saving}
                                className="p-1.5 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-colors shadow-sm disabled:opacity-50 cursor-pointer"
                              >
                                {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <ShieldCheck className="w-3.5 h-3.5" />}
                              </button>
                              <button 
                                onClick={cancelEditing}
                                className="p-1.5 bg-slate-200 text-slate-500 rounded-lg hover:bg-slate-300 transition-colors shadow-sm cursor-pointer"
                              >
          ...
          <div className="flex items-center justify-between py-2 border-t border-slate-50 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className="p-2 rounded-xl border border-white bg-white/40 text-slate-400 disabled:opacity-30 hover:bg-white transition-all shadow-sm cursor-pointer"
              >
                ...
                {[...Array(Math.min(5, totalPages))].map((_, i) => {
                  ...
                  return (
                    <button 
                      ...
                      className={`w-8 h-8 rounded-xl text-[10px] font-black transition-all cursor-pointer ${currentPage === pageNum ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-200' : 'bg-white/40 text-slate-400 hover:bg-white'}`}
                    >
                      {pageNum}
                    </button>
                  );
