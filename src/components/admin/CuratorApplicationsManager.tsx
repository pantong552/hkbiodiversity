...
import { AlertCircle, Check, CheckCircle2, Clock3, Loader2, Search, ShieldX, User, X } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-wrap items-center gap-2" role="tablist" aria-label={language === 'zh' ? '策展人申請狀態' : 'Curator application status'}>
        ...
        <div className="overflow-x-auto rounded-[1.5rem] border border-white bg-white/50 shadow-sm backdrop-blur-xl">
          <table className="w-full min-w-[920px] text-left border-collapse">
            <thead><tr className="border-b border-slate-100 bg-slate-50/80">
              ...
              <th className="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">{language === 'zh' ? '批核時間' : 'Reviewed'}</th>
              {filter !== 'approved' && <th className="px-4 py-3 text-right text-[10px] font-black uppercase tracking-widest text-slate-400">{language === 'zh' ? '操作' : 'Actions'}</th>}
            </tr></thead>
            <tbody>
              {visibleProfiles.map((profile) => (
                <tr key={profile.id} className="border-b border-slate-100/80 align-top transition-colors hover:bg-emerald-50/40">
                  <td className="px-4 py-4"><div className="flex items-center gap-3"><div className="h-9 w-9 shrink-0 overflow-hidden rounded-xl border border-slate-100 bg-slate-50"><>{profile.avatar_url ? <img src={profile.avatar_url} alt="" className="h-full w-full object-cover" onError={(event) => { event.currentTarget.style.display = 'none'; }} /> : <User className="h-full w-full p-2 text-slate-300" />}</></div><div><div className="font-black text-sm text-slate-800">{profile.username || 'Anonymous'}</div><div className="mt-1 text-[11px] text-slate-400">{profile.email}</div></div></div></td>
                  ...
                  <td className="whitespace-nowrap px-4 py-4 text-xs font-medium text-slate-500">{formatDate(profile.curator_application_reviewed_at)}</td>
      ...
      {rejectingProfile && <div className="fixed inset-0 z-[230] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="reject-title"><button type="button" aria-label={language === 'zh' ? '關閉' : 'Close'} onClick={() => setRejectingProfile(null)} className="absolute inset-0 bg-slate-950/50 backdrop-blur-sm cursor-pointer" /><div className="relative w-full max-w-md rounded-[2rem] bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><h2 id="reject-title" className="text-lg font-black text-slate-900">{language === 'zh' ? '拒絕策展人申請' : 'Reject curator application'}</h2><p className="mt-1 text-xs text-slate-500">{rejectingProfile.username || 'Anonymous'}</p></div><button type="button" onClick={() => setRejectingProfile(null)} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100 cursor-pointer"><X className="w-5 h-5" /></button></div><label htmlFor="reject-reason" className="mt-5 block text-xs font-black text-slate-500">{language === 'zh' ? '拒絕原因（必填）' : 'Rejection reason (required)'}</label><textarea id="reject-reason" autoFocus maxLength={500} rows={5} value={rejectionReason} onChange={(event) => setRejectionReason(event.target.value)} className="mt-2 w-full resize-y rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 outline-none focus:border-rose-300 focus:ring-4 focus:ring-rose-500/10" /><div className="mt-1 text-right text-[10px] font-bold text-slate-400">{rejectionReason.length}/500</div><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setRejectingProfile(null)} className="rounded-xl px-4 py-2.5 text-xs font-black text-slate-500 hover:bg-slate-100 cursor-pointer">{language === 'zh' ? '取消' : 'Cancel'}</button><button type="button" onClick={reject} disabled={actionLoading === rejectingProfile.id} className="inline-flex items-center gap-2 rounded-xl bg-rose-600 px-4 py-2.5 text-xs font-black text-white hover:bg-rose-700 cursor-pointer disabled:opacity-50">{actionLoading === rejectingProfile.id && <Loader2 className="w-3.5 h-3.5 animate-spin" />}{language === 'zh' ? '確認拒絕' : 'Confirm reject'}</button></div></div></div>}
    </div>
  );
}
