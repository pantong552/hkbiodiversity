...
export default function UsersManager({ onRequestConfirm }: UsersManagerProps) {
  ...
  // Click Outside Refs
  const roleFilterRef = useClickOutside(() => setShowRoleFilter(false));
  const statusFilterRef = useClickOutside(() => setShowStatusFilter(false));
  ...
  const getRoleBadge = (role: UserRole) => {
    switch (role) {
      case 'admin':
        return <span className="flex items-center gap-1 px-1.5 py-0.5 bg-red-50 text-red-600 rounded-lg text-[8px] font-black uppercase ring-1 ring-red-100"><ShieldCheck className="w-2.5 h-2.5" /> {t('account.role_admin')}</span>;
      case 'curator':
        return <span className="flex items-center gap-1 px-1.5 py-0.5 bg-amber-50 text-amber-600 rounded-lg text-[8px] font-black uppercase ring-1 ring-amber-100"><Shield className="w-2.5 h-2.5" /> {t('account.role_curator')}</span>;
      default:
        return <span className="flex items-center gap-1 px-1.5 py-0.5 bg-slate-50 text-slate-500 rounded-lg text-[8px] font-black uppercase ring-1 ring-slate-100"><UserCircle className="w-2.5 h-2.5" /> {t('account.role_guest')}</span>;
    }
  };

  const getStatusBadge = (status: UserStatus) => {
    if (status === 'blocked') {
      return <span className="px-1.5 py-0.5 bg-slate-900 text-white rounded-md text-[8px] font-black tracking-wider">{t('admin.status_blocked')}</span>;
    }
    return <span className="px-1.5 py-0.5 bg-emerald-50 text-emerald-600 rounded-md text-[8px] font-black tracking-wider ring-1 ring-emerald-100/50">{t('admin.status_active')}</span>;
  };
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-bottom border-slate-100">
                  ...
                  <th ref={roleFilterRef as any} className="px-4 py-2.5 text-[9px] font-black text-slate-400 uppercase tracking-widest relative">
                    ...
                    <AnimatePresence>
                      {showRoleFilter && (
                        <motion.div 
                          ...
                          exit={{ opacity: 0, y: 10 }}
                          className="absolute top-full left-0 mt-1 w-32 bg-white rounded-xl shadow-xl border border-slate-100 p-2 z-20"
                        >
                          {(['all', 'admin', 'curator', 'guest'] as const).map(role => (
                            <button 
                              ...
                              className={`w-full flex items-center justify-between px-2 py-1 text-[10px] font-bold rounded-lg transition-colors ${roleFilter === role ? 'bg-emerald-50 text-emerald-600' : 'text-slate-500 hover:bg-slate-50'}`}
                            >
                  ...
                  <th ref={statusFilterRef as any} className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center relative">
                    <div className="flex items-center justify-center gap-2">
                      <div className="flex items-center gap-2 cursor-pointer group" onClick={() => requestSort('status')}>
                        {t('admin.status')}
                    ...
                    <AnimatePresence>
                      {showStatusFilter && (
                        <motion.div 
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: 10 }}
                          className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-32 bg-white rounded-xl shadow-xl border border-slate-100 p-2 z-20"
                        >
                          {(['all', 'active', 'blocked'] as const).map(status => (
                            <button 
                              ...
                              className={`w-full flex items-center justify-between px-2 py-1 text-[10px] font-bold rounded-lg transition-colors ${statusFilter === status ? 'bg-emerald-50 text-emerald-600' : 'text-slate-500 hover:bg-slate-50'}`}
                            >
                  ...
                  <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">{t('admin.actions')}</th>
                </tr>
              </thead>
                  ...
                  >
                    <td className="px-4 py-2 border-b border-white/10">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="font-black text-slate-800 text-sm">{profile.username || 'Anonymous'}</span>
                      </div>
                    </td>
                    <td className="px-4 py-2 text-xs text-slate-600 border-b border-white/10 font-medium">{profile.email}</td>
                    ...
                    <td className="px-4 py-2 text-right border-b border-white/10">
                      <button onClick={() => handleStatusToggle(profile.id, profile.status)} className={`p-1.5 rounded-xl transition-all ${profile.status === 'active' ? 'text-slate-300 hover:text-rose-500 hover:bg-rose-50' : 'text-emerald-500 bg-emerald-50'}`}>
