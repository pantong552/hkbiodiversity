...
/**
 * 強健的多層次物種正本查詢工具
 * 能完美處理 fauna_/flora_ 前綴、純數字 ID、UUID 以及跨 species/plant_species 表查詢
 */
export async function fetchSpeciesOrPlantRow(rawId: string, preferredTable?: string) {
  if (!rawId) return null;
  const supabase = createClient();
  const cleanId = String(rawId).trim();
  const numericId = cleanId.replace(/^(fauna_|flora_)/, '');

  const isPlant = preferredTable === 'plant_species' || cleanId.startsWith('flora_');
  const primaryTable = isPlant ? 'plant_species' : 'species';
  const secondaryTable = isPlant ? 'species' : 'plant_species';
  ...
  const tryFetch = async (tableName: string) => {
    ...
    // 2. 用完整的 cleanId (如 'fauna_1445' 或 '1445') 比對 taxa_id 或 id
  ...
  // 先在主要資料表搜尋
  let result = await tryFetch(primaryTable);
  if (result) return result;

  // 若主要資料表找不到，跨表在次要資料表搜尋
  result = await tryFetch(secondaryTable);
  return result;
}
