...
export default function HomeClient() {
  ...
  // Fallback嚗???OAuth code ???賢?寥???  useEffect(() => {
  ...
  // 1.1 ?芸??菜葫銝阡??蝔桀???(Share URL support)
  ...
  useEffect(() => {
    ...
    if (speciesId) {
      ...
      if (!isNaN(idNum)) {
        ...
        // ?靽桀儔嚗????Ｗ?皜 URL ?嚗?儐?啗孛??        const newParams = new URLSearchParams(searchParams.toString());
  ...
  const fetchSpecies = useMemo(() => {
    return async () => {
      ...
      try {
        ...
        console.error('fetchSpecies ?航炊:', err);
        setError(err.message || '????航炊');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // 撌脩隡箸??函垢??嚗迨??雿雿??陛?格?摨凝隤?  const filteredAndSortedSpecies = species;

  // Pagination Logic
  const totalPages = Math.ceil(totalResultCount / itemsPerPage);
  const paginatedSpecies = species; // 隡箸??函垢撌脩??末??
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="flex justify-between items-end">
                <div className="max-w-2xl">
                  ...
                  <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-slate-900 tracking-tight leading-[0.9] mb-3 lg:mb-6">
                    {language === 'zh' ? (
                      <>擐葛 <span className="text-emerald-600">?憭見??/span></>
                    ) : (
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-40 text-center bg-red-50 rounded-[3rem] border border-red-100">
                ...
                <h3 className="text-2xl font-black text-red-900 mb-2">鞈?頛憭望?</h3>
      ...
      <footer className="border-t border-slate-100 bg-white py-12 px-6 md:px-8 lg:px-10 xl:px-16">
        <div className="max-w-[1920px] mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex flex-col items-center md:items-start gap-2">
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
              穢 2026 Hong Kong Biodiversity Collective
            </p>
          </div>
